...
NGINX Dashboard URL: `http://EXTERNAL-IP/dashboard.html#upstreams`
...
NGINX Ingress Controller URL: `https://EXTERNAL-IP/`
...
In the terminal window, copy the below text and paste into the terminal.
[source,sh,role=execute]
----
cat << EOF | oc apply -f -
apiVersion: k8s.nginx.org/v1
...
====
To access the Arcadia application, replace `EXTERNAL-IP` with the external IP address of your nginx-ingress:
